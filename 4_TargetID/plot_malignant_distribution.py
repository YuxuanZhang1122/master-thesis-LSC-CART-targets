...
ENSEMBLE_DIR = Path('3_Ensemble/pooledLSC')
OUTPUT_DIR = Path('4_TargetID/Outputs/malignancy_distribution')
label_key = 'consensus_label_6votes'
...
def merge_naldini_datasets(adata):
    """Merge Naldini_V02 and Naldini_V03 into single Naldini dataset"""
    adata = adata.copy()

    source_datasets = adata.obs['source_dataset'].astype(str).values
    merged_datasets = []

    for ds in source_datasets:
        if ds.startswith('Naldini_V02') or ds.startswith('Naldini_V03'):
            suffix = ds.split('_', 2)[-1] if len(ds.split('_')) > 2 else ''
            merged_datasets.append(f'Naldini_{suffix}' if suffix else 'Naldini')
        else:
            merged_datasets.append(ds)

    adata.obs['source_dataset'] = pd.Categorical(merged_datasets)
    return adata

def load_data():
    """Load and concatenate ensemble results from all cell type folders"""
    all_adatas = []

    for cell_type_folder in ['HSC_MPP', 'LMPP', 'Early_GMP']:
        ensemble_path = ENSEMBLE_DIR / cell_type_folder / 'ensemble_results.h5ad'
        if ensemble_path.exists():
            logger.info(f"Loading {ensemble_path}")
            adata = sc.read_h5ad(ensemble_path)
    ...
    combined = merge_naldini_datasets(combined)

    return combined

def calculate_malignancy_stats(adata, label_key):
    """Calculate malignancy distribution statistics by cell type and dataset"""
    rows = []

    consensus = adata.obs[label_key].values
    cell_types = adata.obs['predicted_CellType_Broad'].values
    datasets = adata.obs['source_dataset'].values

    hc_data = {
        'HSC MPP': {'total': 20464, 'lspc': 1, 'hspc': 20463},
        'LMPP': {'total': 5444, 'lspc': 5, 'hspc': 709},
        'Early GMP': {'total': 8818, 'lspc': 82, 'hspc': 328}
    }
    ...
    for cell_type in CELL_TYPES:
        ...
        hc_stats = hc_data.get(cell_type, {'total': 0, 'lspc': 0, 'hspc': 0})
        n_total_overall = n_total + hc_stats['total']
        n_lspc_overall = n_lspc + hc_stats['lspc']
        n_hspc_overall = n_hspc + hc_stats['hspc']

        n_labeled = n_lspc_overall + n_hspc_overall
        pct_lspc = (n_lspc_overall / n_labeled * 100) if n_labeled > 0 else 0

        rows.append({
            'CellType': cell_type,
            'Dataset': 'Overall',
            'Total': n_total_overall,
            'LSPC': n_lspc_overall,
            'HSPC': n_hspc_overall,
        ...
        if cell_type in hc_data:
            hc = hc_data[cell_type]
            n_labeled_hc = hc['lspc'] + hc['hspc']
            pct_lspc_hc = (hc['lspc'] / n_labeled_hc * 100) if n_labeled_hc > 0 else 0
            ...
            rows.append({
                ...
                'HSPC': hc['hspc'],
                'Uncertain': 0,
                'Pct_LSPC': pct_lspc_hc,
                'Pct_HSPC': 100 - pct_lspc_hc
            })

        for dataset in np.unique(datasets):
            mask = (cell_types == cell_type) & (datasets == dataset)
            n_total_ds = mask.sum()
...
def plot_malignancy_by_dataset(stats_df, figsize=(20, 6)):
    """Create malignancy proportion by source dataset"""
...
def plot_celltype_distribution(stats_df, figsize=(20, 6)):
    """Create cell type distribution by source dataset"""
    fig, ax = plt.subplots(figsize=figsize)
...
def get_dataset_colors(datasets):
    ...
    all_colors = {**hc_colors, **ennis_colors, **naldini_colors, **henrik_colors,
                  **petti_colors, **vanGalen_colors, **guo_colors, **abbas_colors}
    ...
    if len(dataset_stats) > 0:
        ...
        def plot_counts_for_celltypes(ax, cell_types_to_plot, title):
            ...
            for i, ct in enumerate(cell_types_to_plot):
                ...
                y_pos = max_height * 1.15 if max_height > 0 else 100
                ax.text(group_center, y_pos, f'Total (excl. HC): {int(sum_lspc):,}',
...
def print_summary_report(stats_df):
    ...
    print("\n" + "="*80)
    print("MALIGNANCY DISTRIBUTION SUMMARY")
    print("="*80 + "\n")
...
def main():
    ...
    stats_df = calculate_malignancy_stats(adata, label_key=label_key)
