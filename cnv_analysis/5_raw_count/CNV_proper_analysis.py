...
reference_profiles = create_reference_profile(adata_filtered, normal_indices, gene_positions, chromosomes)

print("\n" + "="*60)
print("CALCULATING CNV RATIOS")
print("="*60)

cnv_data = calculate_cnv_ratios_from_means(adata_filtered, reference_profiles, normal_indices, malignant_indices)
...
for chr_name in chromosomes:
    ...
    fig.suptitle(f'CNV Analysis - Chromosome {chr_name} (raw)', fontsize=16, y= 0.99)
    ...
    axes[0].set_ylabel('Rar Expression')
    axes[0].legend()
    ...
    axes[1].set_ylabel('Raw Expression')
    axes[1].legend()
    ...
    axes[2].set_ylabel('Raw Expression')
    axes[2].legend()
