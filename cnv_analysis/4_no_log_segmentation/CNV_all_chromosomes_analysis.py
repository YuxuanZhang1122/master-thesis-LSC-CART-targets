...
def visualize_chromosome(chr_name, chr_data, segments, output_dir):
    ...
    fig.suptitle(f'Chr{chr_name} CNV Analysis (normalized, no log) - {len(segments)} segments', fontsize=16, fontweight='bold', y = 0.99)
...
def main():
    ...
    for chr_name in chromosomes:
        ...
        reference_profiles = create_reference_profile(adata_normalized, normal_indices,
                                                    gene_positions, [chr_name])
        
        # Calculate CNV ratios
        cnv_data = calculate_cnv_ratios_from_means(adata_normalized, reference_profiles,
                                                 malignant_indices)
