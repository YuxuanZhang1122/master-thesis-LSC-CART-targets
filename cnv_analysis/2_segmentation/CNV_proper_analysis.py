...
for chr_name in chromosomes:
    ...
    fig.suptitle(f'CNV Analysis - Chromosome {chr_name} (Normalized)', fontsize=16)
    ...
    axes[0].set_ylabel('Log-Normalized Expression')
    axes[0].legend()
    ...
    axes[2].set_ylabel('Log-Normalized Expression')
    axes[2].legend()
    ...
    plt.tight_layout()
    plt.savefig(f'cnv_analysis/chr{chr_name}_individual_cnv_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()